...
#########################
crit1_today <- today_merged %>% 
  mutate(FLAG_strk = case_when((Dog_streak == "wins" | (Dog_streak == "losses" & Dog_streakN < 3)) &
                                 (Fav_streak == "losses" |(Fav_streak == "wins" & Fav_streakN < 3))  ~ "Met",
                               .default = "Not-Met"),
         FLAG_L10 = case_when(as.numeric(Dog_L10) >= .400 & as.numeric(Fav_L10) <= .600 ~ "Met",
                              .default = "Not-Met"),
         FLAG_Away = case_when(as.numeric(Dog_away_per) >= .400 ~ "Met",
                               .default = "Not-Met"))


##########################
# Tracking (teamranking)
###########################
...
#######################################
#This only saves the criteria information we're looking to save. Which we found to be worthless because (see notes about the historical Main_Backtest.Rmd) the information is based on after the game was played.
#Need to also save the model's picks like I did with the NBA System ... began writing the code at the bottom
#######################################
...
picks <- crit1_today %>% 
  rename("HomeTeam" = "home_team",
         "AwayTeam" = "away_team") %>% 
  filter(FLAG_strk == "Met" & FLAG_L10 == "Met" & FLAG_Away == "Met" & SeriesGame == 1) %>% 
  select(Date, HomeTeam, AwayTeam, GameinSeries:SeriesGame, Dog_price, FLAG_strk:FLAG_Away)

# Non_filtered, used especially when picks == 0
non_filter <- crit1_today %>% 
  rename("HomeTeam" = "home_team",
         "AwayTeam" = "away_team") %>% 
  select(Date, HomeTeam:SeriesGame, Dog_price, FLAG_strk:FLAG_Away)

#############################################
# Home Dog - Tracking system with home dogs
#############################################

# Merge with schedule.R and standings.R
home_dog <- alldogs %>%
  left_join(todays_schedule, by = c("home_team" = "HomeTeam")) %>%
  rename("Dog" = "outcomes_name",
         "Dog_price" = "outcomes_price",
         "Date" = "Date.x") %>% 
  select(Date:away_team, HomeSP:AwayWinPer, Dog, Dog_price) %>% 
  ...
  rename("Fav_streak" = "streak",
         ...
         "Fav_diff" = "diff") %>%
  select(-c(ra,rs,wins,losses,away)) %>% 
  filter(home_team == Dog)

# Today's System Picks
home_dog_criteria <- home_dog %>% 
  mutate(FLAG_strk = case_when((Dog_streak == "wins" | (Dog_streak == "losses" & Dog_streakN < 3)) &
                                 (Fav_streak == "losses" |(Fav_streak == "wins" & Fav_streakN < 3))  ~ "Met",
                               .default = "Not-Met"),
         FLAG_L10 = case_when(as.numeric(Dog_L10) >= .400 & as.numeric(Fav_L10) <= .600 ~ "Met",
...
home_dog_picks <- home_dog_criteria %>% 
  rename("HomeTeam" = "home_team",
         "AwayTeam" = "away_team") %>% 
  filter(FLAG_strk == "Met" & FLAG_L10 == "Met" & FLAG_Away == "Met" & SeriesGame == 1) %>% 
  select(Date, HomeTeam, AwayTeam, GameinSeries:SeriesGame, Dog_price, FLAG_strk:FLAG_Away)
